...
{
    void *addr = malloc(APPLY_SPAN_MAP_MEMORY);
    span_map[0] = addr;
}
...
static void central_pool_grow()
{
    ...
    central_pool->pool_end = addr + APPLY_SPAN_HEAD_MEMORY;
    central_pool->free_end = central_pool->pool_end;
    // 扩展span_map
    void *span_map_addr = malloc(APPLY_SPAN_MAP_MEMORY);
    // 计算是哪一段内存
    int memory_index = (addr - RAW_POOL_START) / (APPLY_VIRTUAL_MEMORY);
...
static span_t *acquire_span(int index)
{
    ...
    while (1)
    {
        if (central_pool->free_start <= central_pool->free_end)
        {
            goto finsh;
            break;
        }
...
static void reset_localcache_wc_threshold(uint32_t span_wear_count)
{
    int wc = 0;
...
static void *small_malloc(size_t size)
{
    ...
    while (addr == NULL)
    {
        ...
        if (span == NULL)
        {
            ...
            uint32_t span_wear_count = span->wear_count;
            reset_localcache_wc_threshold(span_wear_count);
        }
