...
static void recalculate_priority(central_pool_t *pool)
{
    ...
    // double w = (double)1 - (double)narea->cpool.max_wc_count / (double)narea->hp_table.memory_wear_limit;
...
static void span_back_to_centralpool(span_t *span)
{
    ...
    if (span->wear_count > central_pool->max_wc_count)
    {
        ...
        list_add_tail(&span->span_free_list, &central_pool->l_free_list[index].free_list);
        central_pool->l_free_list[index].max_wear_page_index = &span->span_free_list;
        for (int i = 0; i < LARGE_BLOCK_NUM; i++)
        {
            ...
            central_pool->l_free_list[index].max_wear_page_index = NULL;
        }
    }
    else if (span->wear_count == central_pool->max_wc_count)
    {
        list_add_tail(&span->span_free_list, &central_pool->l_free_list[index].free_list);
        if (central_pool->l_free_list[index].max_wear_page_index == NULL)
        {
            central_pool->l_free_list[index].max_wear_page_index = &span->span_free_list;
        }
    }
    else
    {
        ...
        {
            list_add(&span->span_free_list, &central_pool->l_free_list[index].max_wear_page_index->prev);
        }
    }
}
...
static void central_pool_init(central_pool_t *central_pool, nvm_area_t *nvm_area, int flag)
{
    ...
    */
    ...
    for (int i = 0; i < LARGE_BLOCK_NUM; i++)
    {
        INIT_LIST_HEAD(&central_pool->l_free_list[i].free_list);
        central_pool->l_free_list[i].max_wear_page_index = NULL;
    }
...
static span_t *span_split(list_head *list_node, int cur_index, int target_index, central_pool_t *central_pool)
{
    ...
    span_s->wear_count = span->wear_count;
    ...
    //     if (list_index_s > 255)
    //     {
    //         list_add_tail(&span->span_free_list, &central_pool->l_free_list[LARGE_BLOCK_NUM - 1].free_list);
    //     }
    //     else
    //     {
    //         list_add_tail(&span->span_free_list, &central_pool->l_free_list[list_index_s].free_list);
    //     }
    // }
    // else
    // {
    //     if (list_index_s > 255)
    //     {
    //         list_add(&span->span_free_list, &central_pool->l_free_list[LARGE_BLOCK_NUM - 1].free_list);
    //     }
    //     else
    //     {
    //         list_add(&span->span_free_list, &central_pool->l_free_list[list_index_s].free_list);
    //     }
    // }
    return span;
}

void init_span_f(span_t *span, central_pool_t *pool, void *span_start)
{
    span->pool = pool;
    span->wear_count = 0;
    span->free_start = span_start;
}
...
static span_t *acquire_span(central_pool_t *central_pool, int index)
{
    ...
    while (1)
    {
        ...
        {
            if (!list_empty(&central_pool->l_free_list[index].free_list))
            {
                if (central_pool->l_free_list[index].max_wear_page_index != NULL && central_pool->l_free_list[index].max_wear_page_index == central_pool->l_free_list[index].free_list.next)
                {
                    if (central_pool->l_free_list[index].free_list.next->next == &central_pool->l_free_list[index].free_list)
                    {
                        central_pool->l_free_list[index].max_wear_page_index = NULL;
                    }
                    else
                    {
                        central_pool->l_free_list[index].max_wear_page_index = central_pool->l_free_list[index].free_list.next->next;
                    }
                }
                span = list_entry(central_pool->l_free_list[index].free_list.next, span_t, span_free_list);
                list_del(&span->span_free_list);
                return span;
            }
            else
            {
                ...
                {
                    if (!list_empty(&central_pool->l_free_list[i].free_list))
                    {
                        if (central_pool->l_free_list[i].max_wear_page_index != NULL && central_pool->l_free_list[i].max_wear_page_index == central_pool->l_free_list[i].free_list.next)
                        {
                            if (central_pool->l_free_list[i].free_list.next->next == &central_pool->l_free_list[i].free_list)
                            {
                                central_pool->l_free_list[i].max_wear_page_index = NULL;
                            }
                            else
                            {
                                central_pool->l_free_list[i].max_wear_page_index = central_pool->l_free_list[i].free_list.next->next;
                            }
                        }
                        if (i == LARGE_BLOCK_NUM - 1)
                        {
                            span = span_split(central_pool->l_free_list[i].free_list.next, 256, index, central_pool);
                        }
                        else
                        {
                            span = span_split(central_pool->l_free_list[i].free_list.next, i, index, central_pool);
                        }
                        list_del(&span->span_free_list);
                        return span;
                    }
    ...
    finsh:
        ...
        init_span_f(span, central_pool, central_pool->free_start);
...
void reset_localcache_wc_threshold(central_pool_t *central_pool, uint64_t span_wear_count)
{
    int wc = 0;
...
span_t *acquire_span_from_centralpool(central_pool_t *cpool, int index)
{
    ...
    reset_localcache_wc_threshold(cpool, span_max_wear_count);
    return span;
}
...
void *span_clock_allocate(span_t *span)
{
    ...
    void *addr = (void *)span->free_start + (cur_pos - 1) * (span->span_category + 1) * MIN_STORE_UNIT;
    if (cur_pos == last_pos)
    {
        span->wear_count += 1;
    }
