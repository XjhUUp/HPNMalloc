...
static void *small_malloc(size_t size)
{
    ...
    while (addr == NULL)
    {
        ...
        if (span == NULL)
        {
            ...
            while (true)
            {
                ...
                {
                    if (nvm_index == nvm_list_area.nvm_index )
                    {
                        nvm_index = 0;
                    }else{
                        nvm_index+=1;
    ...
    nvm_area->hp_table.used_mc += size;
    init_priority(nvm_area);
    pthread_mutex_unlock(&nvm_area->lock);
...
void *cpfree(void *ptr)
{
    ...
    central_pool_t *cpool = span->pool;
    nvm_area_t *nvm_area = cpool->nvm_area;
    pthread_mutex_lock(&nvm_area->lock);
    if (span->span_category == 256)
    {
       nvm_area->hp_table.used_mc -= span->sl_block_t.page_num * PAGE_SIZE;
    }
    else
    {
        nvm_area->hp_table.used_mc -= (span->span_category + 1) * MIN_STORE_UNIT;
    }
    init_priority(span->pool->nvm_area);
    pthread_mutex_unlock(&nvm_area->lock);
