...
nvm_area_t *make_init_nvm_area(uint32_t hardware_mc, uint32_t remain_mc, uint64_t memory_wear_limit, uint64_t memory_wear_remain, float unit_capacity_mp, float wear_weight)
{
    ...
    n_area->hp_table.remain_mc = remain_mc;
...
void init_priority(nvm_area_t *n_area)
{
    ...
    wear_cost = table->memory_wear_remain / table->unit_capacity_mp; // NVM的剩余磨损次数/单位内存价格
    economic_cost = table->remain_mc / table->unit_capacity_mp;      // NVM的剩余容量大小/单位内存价格
    priority = n_area->hp_table.wear_weight * wear_cost * economic_cost;
